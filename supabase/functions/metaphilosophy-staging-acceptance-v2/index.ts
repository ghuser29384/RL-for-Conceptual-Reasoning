...
const admin = createClient(supabaseUrl, serviceRoleKey, {
  ...
  global: { headers: { "x-metaphilosophy-component": "staging-acceptance-v2" } },
});
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (statusCode >= 500) console.error("metaphilosophy_staging_acceptance_v2_error", error);
...
async function status(exactReleaseSha: string, claims: JWTPayload) {
  ...
  const restore = await latestRestoreReadback();
...
async function restoreAndVerify({ events, expectedEventCount, expectedHeadHash, expectedBackupSha256, exactReleaseSha, claims }) {
  ...
  const restoreRunId = await deterministicUuid(`${exactReleaseSha}:${backupSha256}`);
  const { data, error } = await admin.rpc("metaphilosophy_staging_restore_run_load", {
    p_restore_run_id: restoreRunId,
    p_exact_release_sha: exactReleaseSha,
    p_backup_sha256: backupSha256,
    p_expected_event_count: events.length,
    p_expected_head_hash: verification.headHash,
    p_events: events,
  });
  if (error) throw new Error(`restore run load failed: ${error.message}`);
  const row = data?.[0];
  if (String(row?.restore_run_id ?? "") !== restoreRunId) throw new Error("restore run ID did not match the requested release-scoped target");
  if (Number(row?.restored_count ?? 0) !== events.length) throw new Error("restore run count did not match the backup");
  if (String(row?.head_hash ?? "") !== verification.headHash) throw new Error("restore run head did not match the backup");

  const existing = await loadRestoreEvents(restoreRunId);
  const existingVerification = await verifyEventChain(existing);
  if (canonicalStringify(existing) !== canonicalStringify(events)) {
    throw acceptanceError(409, "restore_target_differs", "The release-scoped restore target contains a different event chain.");
  }
  const readback = await restoreReadback(restoreRunId);
...
async function latestRestoreReadback() {
  const { data, error } = await admin
    .from("metaphilosophy_staging_restore_runs")
    .select("id,exact_release_sha,backup_sha256,event_count,head_hash,research_ratings_authorized,created_at")
    .order("created_at", { ascending: false })
    .limit(1);
  if (error) throw new Error(`restore-run header read failed: ${error.message}`);
  const latest = data?.[0];
  if (!latest) return { ...emptyReadback(), restoreRunId: null, exactReleaseSha: null, backupSha256: null };
  const readback = await restoreReadback(latest.id);
  return {
    ...readback,
    restoreRunId: latest.id,
    exactReleaseSha: latest.exact_release_sha,
    backupSha256: latest.backup_sha256,
    createdAt: latest.created_at,
  };
}

async function restoreReadback(restoreRunId: string) {
  const { data, error } = await admin.rpc("metaphilosophy_staging_restore_run_readback", {
    p_restore_run_id: restoreRunId,
  });
  if (error) throw new Error(`restore-run readback failed: ${error.message}`);
  return normalizeReadback(data?.[0]);
}

async function loadRestoreEvents(restoreRunId: string) {
  const { data, error } = await admin
    .from("metaphilosophy_staging_restore_run_events")
    .select("sequence,event_id,event_type,aggregate_id,actor_id,payload,created_at,prev_hash,event_hash")
    .eq("restore_run_id", restoreRunId)
    .order("sequence", { ascending: true });
  if (error) throw new Error(`restore-run event read failed: ${error.message}`);
...
function emptyReadback() {
  return {
    ...
    sequenceGapCount: 0,
    previousHashMismatchCount: 0,
    duplicateEventIdCount: 0,
    duplicateEventHashCount: 0,
...
function assertReadback(readback, eventCount, headHash) {
  if (
    ...
    readback.researchRatingsAuthorized !== false
  ) {
    throw new Error("restore-run database readback failed its integrity invariants");
  }
}

function assertSyntheticMetadata(metadata) {
  if (
    Number(metadata?.schema_version) !== 5 ||
    metadata?.purpose !== "synthetic_rehearsal_only" ||
    metadata?.research_ratings_authorized !== false
  ) {
    throw new Error("staging metadata does not preserve the schema-v5 synthetic-only authorization boundary");
  }
}
...
async function verifyVercelPreviewToken(token: string): Promise<JWTPayload> {
  ...
  if (!payload) {
    console.warn("metaphilosophy_staging_acceptance_v2_oidc_rejected", lastError instanceof Error ? lastError.message : String(lastError));
