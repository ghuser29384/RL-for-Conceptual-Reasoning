...
def iter_files(roots: Iterable[pathlib.Path], recursive: bool) -> Iterable[pathlib.Path]:
    for root in roots:
        ...
        try:
            if recursive:
                yield from (path for path in root.rglob("*.md") if path.is_file())
            else:
                yield from (path for path in root.glob("*.md") if path.is_file())
...
def detect(roots: list[pathlib.Path], recursive: bool) -> dict:
    ...
    matches.sort(key=lambda item: (item["version"], item["mtime"] or 0, item["path"]), reverse=True)
    if not matches:
        return {
            "found": False,
            "version": None,
...
def main(argv: list[str]) -> int:
    ...
    roots = [pathlib.Path(root).expanduser().resolve() for root in args.roots] if args.roots else default_roots()
