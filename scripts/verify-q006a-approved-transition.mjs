...
export async function verifyQ006aApprovedTransition(root = resolve(import.meta.dirname, "..")) {
  ...
  const [historical, effective, state, interpretation, slate, emailPacket, excerpt] = await Promise.all([
  ...
  assert.equal(historical.ledger_id, "metaphilosophy-pilot-readiness-v1-2026-07-30");
  assert.equal(historical.authorization_state?.q006a?.status, "pending_project_owner_decision");

  assert.equal(effective.ledger_id, "metaphilosophy-pilot-readiness-v2-2026-08-01");
  assert.equal(effective.ledger_version, 2);
  assert.equal(effective.supersedes?.ledger_id, historical.ledger_id);
  ...
  assert.equal(effective.q006a?.approved_at, "2026-08-01T11:34:32Z");
  ...
  assert.equal(gates.get("R-01")?.passed_at, "2026-08-01T11:34:32Z");
  ...
  assert.equal(state.approved_at, "2026-08-01T11:34:32Z");
  ...
  for (const path of EXPECTED_ARTIFACTS) await access(resolve(root, path), constants.R_OK);
