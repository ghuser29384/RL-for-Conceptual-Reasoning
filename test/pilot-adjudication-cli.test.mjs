...
const scriptPath = resolve(root, "scripts/pilot-adjudication.mjs");
const datasetPath = resolve(root, "test/fixtures/pilot-rating-analysis-synthetic.json");
const policyPath = resolve(root, "test/fixtures/pilot-analysis-policy-adjudication-synthetic.json");
...
test("runs adjudication case generation as a public-summary-only simulation", async () => {
  ...
  assert.equal(summary.report_version, "pilot-adjudication-case-public-summary-v1");
  assert.equal(summary.mode, "simulation");
  assert.equal(summary.counts.cases, 2);
...
test("refuses to write controlled case artifacts in simulation mode", async () => {
  await assert.rejects(
    execFileAsync(
      process.execPath,
      [scriptPath, datasetPath, policyPath, controlPath, "--controlled-output", "/tmp/forbidden-pilot-adjudication-cases.json"],
      { cwd: root, maxBuffer: 4 * 1024 * 1024 }
    ),
    (error) => /Simulation mode does not write controlled adjudication case files/.test(`${error.stderr}${error.stdout}`)
  );
});
